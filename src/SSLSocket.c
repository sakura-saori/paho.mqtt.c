...
int SSLSocket_createContext_TLCP(networkHandles *net, MQTTClient_SSLOptions *opts);
...
int SSLSocket_createContext(networkHandles *net, MQTTClient_SSLOptions *opts)
{
	...
	if (net->ctx == NULL)
	{
		if (opts->dprivateKey || opts->dkeyStore)
		{
			rc = SSLSocket_createContext_TLCP(net, opts);
			return rc;
		}
...
int SSLSocket_createContext_TLCP(networkHandles *net, MQTTClient_SSLOptions *opts)
{
	...
	{
#if (OPENSSL_VERSION_NUMBER >= 0x10100000L)
		net->ctx = SSL_CTX_new(CNTLS_client_method());
#else
		int sslVersion = MQTT_SSL_VERSION_DEFAULT;
		if (opts->struct_version >= 1)
			sslVersion = opts->sslVersion;
	...
	{
		// sign cert
		if ((rc = SSL_CTX_use_certificate_file(net->ctx, opts->keyStore, SSL_FILETYPE_PEM)) != 1)
		{
			SSLSocket_error("SSL_CTX_use_certificate_file load Signature certificate failed", NULL, net->socket, rc, opts->ssl_error_cb, opts->ssl_error_context);
			goto free_ctx;
		}
		// sign private key
		if ((rc = SSL_CTX_use_PrivateKey_file(net->ctx, opts->privateKey, SSL_FILETYPE_PEM)) != 1)
		{
			SSLSocket_error("SSL_CTX_use_certificate_file load Signature key failed", NULL, net->socket, rc, opts->ssl_error_cb, opts->ssl_error_context);
			goto free_ctx;
		}
		if ((rc = SSL_CTX_check_private_key(net->ctx)) != 1)
		{
			SSLSocket_error("SSL_CTX_check_private_key check Signature key failed", NULL, net->socket, rc, opts->ssl_error_cb, opts->ssl_error_context);
			goto free_ctx;
		}
	}
	...
	{
		// encrypt cert
		if ((rc = SSL_CTX_use_enc_certificate_file(net->ctx, opts->dkeyStore, SSL_FILETYPE_PEM)) != 1)
		{
			SSLSocket_error("SSL_CTX_use_enc_certificate_file load Encrypt certificate failed", NULL, net->socket, rc, opts->ssl_error_cb, opts->ssl_error_context);
			goto free_ctx;
		}
		// encrypt private key
		if ((rc = SSL_CTX_use_enc_PrivateKey_file(net->ctx, opts->dprivateKey, SSL_FILETYPE_PEM)) != 1)
		{
			SSLSocket_error("SSL_CTX_use_enc_PrivateKey_file load Encrypt Key failed", NULL, net->socket, rc, opts->ssl_error_cb, opts->ssl_error_context);
			goto free_ctx;
		}
		if ((rc = SSL_CTX_check_enc_private_key(net->ctx)) != 1)
		{
			SSLSocket_error("SSL_CTX_check_enc_private_key check Encrypt Key failed", NULL, net->socket, rc, opts->ssl_error_cb, opts->ssl_error_context);
			goto free_ctx;
		}
	}
	...
	if ((rc = SSL_CTX_check_private_key(net->ctx)) != 1)
	{
		SSLSocket_error("SSL_CTX_check", NULL, net->socket, rc, NULL, NULL);
		goto free_ctx;
	}
